...
async function handleStats(dbPath: string, args: string[]) {
  ...
  if (agentId) {
    ...
    for (const activity of summary.recentActivity.slice(0, 7)) {
      console.log(
        `  ${activity.timeGroup}: ${activity.count} executions (${activity.successRate}% success)`,
      );
    }
  } else {
    // System overview
    ...
    for (const trigger of overview.triggerStats.slice(0, 5)) {
      console.log(
        `  ${trigger.trigger}: ${trigger.count} executions (${trigger.successRate}% success)`,
      );
    }
  }
  ...
  switch (type) {
    ...
    case 'trends': {
      ...
      if (trends.error) {
        ...
        console.log(
          `  Execution Volume: ${trends.trends.executionVolume.direction} (${trends.trends.executionVolume.slope})`,
        );
        console.log(
          `  Execution Time: ${trends.trends.executionTime.direction} (${trends.trends.executionTime.slope})`,
        );
        console.log(
          `  Success Rate: ${trends.trends.successRate.direction} (${trends.trends.successRate.slope})`,
        );
      }
      break;
    }
