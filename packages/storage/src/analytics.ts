...
export class StorageAnalytics {
    ...
    public detectAnomalies(agentId: string, days = 30): any[] {
        ...
        const successRates = hourlyData.map(d => d.count > 0 ? (d.successful / d.count) * 100 : 100);
        ...
        for (const data of hourlyData) {
            ...
            if (countZScore > 2) {
                anomalies.push({
                    ...
                    description: data.count > countMean ? 'Unusually high execution volume' : 'Unusually low execution volume'
                });
            }
            ...
            if (successRate < successMean - 20 && data.count >= 3) { // At least 3 executions
                anomalies.push({
                    type: 'success_rate',
                    hour: data.hour,
    ...
    public getFailurePatterns(days = 30): any {
        ...
        // Analyze patterns
        const byAgent = failures.reduce((acc, failure) => {
            acc[failure.agentId] = (acc[failure.agentId] || 0) + 1;
            return acc;
        }, {} as Record<string, number>);

        const byTrigger = failures.reduce((acc, failure) => {
            acc[failure.trigger] = (acc[failure.trigger] || 0) + 1;
            return acc;
        }, {} as Record<string, number>);

        const byHour = failures.reduce((acc, failure) => {
    ...
    private calculateTrend(values: number[]): { direction: 'up' | 'down' | 'stable', slope: number, confidence: number } {
    ...
    private getMaxKey(obj: Record<string | number, number>): string {
        return Object.entries(obj).reduce((max, [key, value]) =>
            value > (obj[max] || 0) ? key : max, Object.keys(obj)[0] || '');
    }
}