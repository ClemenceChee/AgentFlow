...
export function extractOperatorPatterns(
  ...
  const collaborationEvents = timeline.filter(item =>
  ...
  if (collaborationEvents.length > totalSessions * 0.2) {
    patterns.push({
      ...
      recommendations: ['Consider team knowledge sharing sessions', 'Document collaborative workflows']
    });
  }
...
export function categorizeActivity(trace: OrganizationalTrace): {
  ...
  const devScore = devKeywords.filter(keyword =>
    name.includes(keyword) || agentId.includes(keyword)
  );
  ...
  const category = Object.entries(scores).find(([, score]) => score === maxScore)?.[0] as any || 'other';
...
export function calculateTeamMetrics(
  ...
  const teamTraces = traces.filter(trace =>
    trace.operatorContext?.teamId === teamId
  );

  if (teamTraces.length === 0) {
    return { teamId, metrics: { successRate: 0, averageExecutionTime: 0, totalExecutions: 0, activeOperators: 0, collaborationScore: 0 } };
  }
  ...
  const totalExecutionTime = teamTraces.reduce((sum, trace) =>
    sum + (trace.endTime - trace.startTime), 0
  );
  const averageExecutionTime = totalExecutionTime / teamTraces.length;

  const uniqueOperators = new Set(
    teamTraces
      .map(trace => trace.operatorContext?.operatorId)
      .filter(Boolean)
  );

  // Simple collaboration score based on unique operators
  const collaborationScore = Math.min(uniqueOperators.size / Math.max(teamTraces.length * 0.5, 1), 1);
...
export function generateInsights(intelligence: OrganizationalIntelligence): Array<{
  ...
  const collaborationRate = intelligence.operatorInsights.collaborationEvents /
...
export function createDisplaySummary(traces: OrganizationalTrace[]): {
  ...
  const uniqueOperators = new Set(
    traces.map(t => t.operatorContext?.operatorId).filter(Boolean)
  ).size;

  const uniqueTeams = new Set(
    traces.map(t => t.operatorContext?.teamId).filter(Boolean)
  ).size;
