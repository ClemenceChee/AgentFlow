      ...
      <tbody className="bg-white divide-y divide-gray-200">
        {apis.map((api) => (
          <tr key={`${api.method}:${api.endpoint}`}>
            ...
            <td
              className={`px-3 py-2 text-sm ${
                api.errorCount / api.requestCount > 0.1
                  ? 'text-red-600 font-medium'
                  : 'text-gray-500'
              }`}
            >
...
export const PerformanceDashboard: React.FC<PerformanceDashboardProps> = memo(
  ({ autoRefresh = true, refreshInterval = 5000, className = '' }) => {
    const [report, setReport] = useState<PerformanceReport | null>(null);
    const [isEnabled, setIsEnabled] = useState(false);
    const [activeTab, setActiveTab] = useState<'overview' | 'components' | 'apis' | 'warnings'>(
      'overview',
    );
    ...
    useEffect(() => {
      setIsEnabled(
        process.env.NODE_ENV === 'development' ||
          localStorage.getItem('org-perf-monitor') === 'true',
      );
    }, []);
