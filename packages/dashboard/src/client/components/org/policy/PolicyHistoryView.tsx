...
export function PolicyHistoryView({
  ...
  const generateMockHistoryData = (range: string): PolicyHistoryData[] => {
    ...
    for (let i = config.buckets - 1; i >= 0; i--) {
      ...
      data.push({
        timestamp,
        compliance: score > 0.8 ? 'compliant' : score > 0.6 ? 'warning' : score > 0.4 ? 'violation' : 'pending',
        score,
        violationCount,
  ...
  const detectViolationPatterns = (data: PolicyHistoryData[]): ViolationPattern[] => {
    ...
    const avgHourlyViolations = Array.from(hourlyViolations.values()).reduce((sum, count) => sum + count, 0) / 24;

    if (maxHourlyViolations > avgHourlyViolations * 2) {
      const peakHour = Array.from(hourlyViolations.entries()).find(([, count]) => count === maxHourlyViolations)?.[0];
      patterns.push({
        id: 'temporal-pattern',
  ...
  const complianceMetrics = useMemo(() => {
    ...
    const previousScore = historyData.length > 1 ? historyData[historyData.length - 2]?.score || 0 : currentScore;
    ...
    const avgComplianceScore = historyData.reduce((sum, item) => sum + item.score, 0) / historyData.length;
    ...
    return {
      ...
      categoryTotals,
      trendDirection: scoreChange > 0.02 ? 'improving' : scoreChange < -0.02 ? 'declining' : 'stable'
    };
  }, [historyData]);
      ...
      <div className={containerClasses}>
        ...
        <div className="org-card__content">
          <div className="policy-history-loading">
            ...
            <div className="policy-history-loading-text">
              Loading policy compliance history...
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={containerClasses}>
        ...
        <div className="org-card__content">
          <div className="policy-history-error">
            ...
            <div className="policy-history-error__message">
              {error}
            </div>
          </div>
        </div>
    ...
    <div className={containerClasses}>
      <div className="org-card__header">
        <div className="org-card__title">
          ...
          {teamId && (
            <span className="policy-history-view__team">
              • Team {teamId.substring(0, 8)}
            </span>
          )}
        </div>
        <div className="org-card__subtitle">
          {TIME_BUCKET_CONFIG[timeRange].label}
        </div>
      </div>
      ...
      <div className="org-card__content">
          ...
          <div className="policy-history-summary">
            <div className="policy-history-metric">
              <div className="policy-history-metric__value">
                {complianceMetrics.currentScore}%
              </div>
              <div className="policy-history-metric__label">
                Current Score
              </div>
              {complianceMetrics.scoreChange !== 0 && (
                <div className={`policy-history-metric__change ${complianceMetrics.trendDirection}`}>
                  {complianceMetrics.scoreChange > 0 ? '+' : ''}{complianceMetrics.scoreChange}%
                </div>
              )}
            </div>
        ...
        {showComplianceEvolution && historyData.length > 0 && (
          <div className="policy-history-chart">
            <div className="policy-history-chart__header">
              <div className="policy-history-chart__title">
            ...
            <div className="policy-history-chart__container">
              <div className="policy-history-chart__timeline">
                {historyData.map((item, index) => {
                  ...
                  const color = item.compliance === 'compliant' ? 'var(--success)' :
                               item.compliance === 'warning' ? 'var(--warn)' :
                               item.compliance === 'violation' ? 'var(--fail)' : 'var(--t3)';
            ...
            <div className="policy-history-patterns-list">
                  ...
                  >
                    <div className="policy-history-pattern__header">
                      <div className="policy-history-pattern__type">
                        ...
                        <span className="policy-history-pattern__label">
                          {config.label}
                        </span>
                      </div>
                    ...
                    <div className="policy-history-pattern__description">
                      {pattern.description}
                    </div>
              ...
              {violationPatterns.length > (compact ? 3 : 6) && (
                <div className="policy-history-patterns-more">
                  +{violationPatterns.length - (compact ? 3 : 6)} more pattern{violationPatterns.length - (compact ? 3 : 6) !== 1 ? 's' : ''}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="policy-history-categories">
            ...
            <div className="policy-history-category-bars">
              {Object.entries(complianceMetrics.categoryTotals)
                ...
                .slice(0, compact ? 3 : 6)
                .map(([category, count]) => {
                  const percentage = complianceMetrics.totalViolations > 0 ?
                    (count / complianceMetrics.totalViolations) * 100 : 0;
                  ...
                  return (
                    <div key={category} className="policy-history-category-bar">
                      <div className="policy-history-category-bar__info">
                        ...
                        <span className="policy-history-category-bar__count">
                          {count}
                        </span>
                      </div>
        ...
        {historyData.length === 0 && (
          <div className="policy-history-empty">
            ...
            <div className="policy-history-empty__message">
              No policy history data available
            </div>
