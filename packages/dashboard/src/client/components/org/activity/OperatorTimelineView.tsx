...
import { Clock, User, CheckCircle, XCircle, AlertCircle, Play, Pause, ArrowRight, TrendingUp, Filter } from 'lucide-react';
...
export interface ActivityEvent {
  ...
  readonly eventType: 'task-start' | 'task-complete' | 'task-failed' | 'collaboration' | 'break' | 'context-switch' | 'problem-solving' | 'review';
  readonly title: string;
  readonly description?: string;
  readonly duration?: number; // For events with duration
  readonly outcome?: 'success' | 'failure' | 'partial' | 'cancelled';
  readonly category: 'coding' | 'debugging' | 'review' | 'research' | 'meeting' | 'planning' | 'documentation';
...
export interface ProductivityMetric {
  readonly metric: 'tasks-completed' | 'avg-task-duration' | 'success-rate' | 'context-switches' | 'collaboration-ratio' | 'problem-solving-efficiency';
...
const getEventTypeColor = (type: ActivityEvent['eventType']) => {
  switch (type) {
    case 'task-start': return 'org-text-info';
    case 'task-complete': return 'org-text-success';
    case 'task-failed': return 'org-text-error';
    case 'collaboration': return 'org-text-primary';
    case 'break': return 'org-text-muted';
    case 'context-switch': return 'org-text-warning';
    case 'problem-solving': return 'org-text-info';
    case 'review': return 'org-text-secondary';
...
const getCategoryColor = (category: ActivityEvent['category']) => {
  switch (category) {
    case 'coding': return 'org-bg-blue-100';
    case 'debugging': return 'org-bg-red-100';
    case 'review': return 'org-bg-purple-100';
    case 'research': return 'org-bg-green-100';
    case 'meeting': return 'org-bg-orange-100';
    case 'planning': return 'org-bg-indigo-100';
    case 'documentation': return 'org-bg-gray-100';
    default: return 'org-bg-surface';
  }
};
...
const getDifficultyColor = (difficulty?: ActivityEvent['difficulty']) => {
  switch (difficulty) {
    ...
    case 'complex': return 'org-text-error';
    default: return 'org-text-muted';
  }
};
          ...
          {sortedEvents.map((event, index) => {
            ...
            return (
              <div key={event.id} className="relative flex items-start">
                <div className={`flex-shrink-0 w-16 h-8 flex items-center justify-center z-10 ${getCategoryColor(event.category)} rounded-full`}>
                ...
                <div className="flex-1 ml-4">
                  ...
                  >
                    ...
                    {event.outcome && (
                      <div className={`inline-flex items-center space-x-1 org-badge org-badge-xs ${
                        event.outcome === 'success' ? 'org-badge-success' :
                        event.outcome === 'failure' ? 'org-badge-error' :
                        event.outcome === 'partial' ? 'org-badge-warning' :
                        'org-badge-secondary'
                      }`}>
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-1">
                                {event.tools.map(tool => (
                                  <span key={tool} className="org-badge org-badge-secondary org-badge-xs">
                                    {tool}
                                  </span>
                                ))}
                              </div>
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-1">
                                {event.collaborators.map(collaborator => (
                                  <span key={collaborator} className="org-badge org-badge-info org-badge-xs">
                ...
                {!isLast && event.duration && (
                  <div className="absolute left-8 top-8" style={{ height: `${Math.min(event.duration / 60000, 120)}px` }}>
...
const CategorizedView: React.FC<{
  ...
  const eventsByCategory = timeline.events.reduce((acc, event) => {
    ...
    <div className="org-timeline-categorized">
      <div className="space-y-6">
        {categories.map(category => {
          const categoryEvents = eventsByCategory[category].sort((a, b) => b.timestamp - a.timestamp);
          const totalDuration = categoryEvents.reduce((sum, event) => sum + (event.duration || 0), 0);
          const successRate = categoryEvents.filter(e => e.outcome === 'success').length / categoryEvents.length;
            ...
            <div key={category} className="org-category-section">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="org-text-sm org-text-muted">
                  ...
                  {successRate > 0 && (
                    <span className={`ml-2 ${successRate >= 0.8 ? 'org-text-success' : successRate >= 0.6 ? 'org-text-info' : 'org-text-warning'}`}>
                    ...
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      {event.outcome && (
                        <div className={`w-2 h-2 rounded-full ${
                          event.outcome === 'success' ? 'org-bg-success' :
                          event.outcome === 'failure' ? 'org-bg-error' :
                          event.outcome === 'partial' ? 'org-bg-warning' :
                          'org-bg-muted'
                        }`}></div>
                      )}
                    </div>
      ...
      <div className="org-card-inner">
        ...
        <div className="space-y-2">
          {timeline.peakProductivityHours.map(hour => (
            <div key={hour} className="flex items-center justify-between">
              <span className="org-font-medium">{hour}:00 - {hour + 1}:00</span>
        ...
        <div className="mt-4 pt-4 org-border-t">
          ...
          <div className="space-y-1">
            {Object.entries(
              timeline.events.reduce((acc, event) => {
        ...
        {['morning', 'afternoon', 'evening'].map(period => {
          ...
          return (
            <div key={period} className="text-center">
              ...
              <div className="org-font-semibold">{completedTasks}/{totalTasks}</div>
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-start space-x-3">
                <div className={`w-10 h-10 rounded-lg ${getCategoryColor(event.category)} flex items-center justify-center`}>
                  <div className={getEventTypeColor(event.eventType)}>
                    {getEventTypeIcon(event.eventType)}
                  </div>
                </div>
...
export const OperatorTimelineView: React.FC<OperatorTimelineViewProps> = ({
      ...
      try {
        ...
        const events: ActivityEvent[] = Array.from({ length: 25 }, (_, i) => {
          ...
          const eventType: ActivityEvent['eventType'] =
            ['task-start', 'task-complete', 'task-failed', 'collaboration', 'break', 'context-switch', 'problem-solving', 'review'][
              Math.floor(Math.random() * 8)
            ] as ActivityEvent['eventType'];

          const category: ActivityEvent['category'] =
            ['coding', 'debugging', 'review', 'research', 'meeting', 'planning', 'documentation'][
          ...
          return {
            ...
            description: Math.random() > 0.5 ? `Detailed description for event ${i + 1} involving ${category} work` : undefined,
            duration: Math.random() > 0.3 ? 300000 + Math.random() * 7200000 : undefined, // 5min - 2h
            outcome: Math.random() > 0.2 ? (['success', 'failure', 'partial', 'cancelled'][Math.floor(Math.random() * 4)] as ActivityEvent['outcome']) : undefined,
            category,
            difficulty: Math.random() > 0.4 ? (['low', 'medium', 'high', 'complex'][Math.floor(Math.random() * 4)] as ActivityEvent['difficulty']) : undefined,
            tools: Math.random() > 0.6 ? ['VS Code', 'Git', 'Docker', 'npm', 'TypeScript'].slice(0, Math.floor(Math.random() * 3) + 1) : undefined,
            collaborators: Math.random() > 0.7 ? Array.from({ length: Math.floor(Math.random() * 3) + 1 }, () =>
  ...
  return (
    <div className={`org-timeline-view org-timeline-${mode} ${className}`}>
      ...
      {mode === 'detailed' && (
        <DetailedView timeline={timeline} onSelectEvent={onSelectEvent} />
      )}
    </div>
  );
};
