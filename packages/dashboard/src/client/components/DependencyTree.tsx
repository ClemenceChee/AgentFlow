...
const TYPE_COLORS: Record<string, string> = {
  ...
  embedder: '#f0883e', watcher: '#79c0ff', writer: '#56d364', default: '#8b949e',
};
...
function NodeRow({ node, trace, depth }: { node: TraceNode; trace: FullTrace; depth: number }) {
  ...
  return (
    <>
      <div className={`dt-node ${fail ? 'dt-node--fail' : ''}`} style={{ paddingLeft: depth * 20 + 8 }}>
        <span style={{ color, marginRight: 6 }}>{icon}</span>
        <span className="dt-node__name">{node.name}</span>
        <span className="dt-node__type" style={{ color }}>{node.type}</span>
        <span className="dt-node__dur">{fmtDur(dur)}</span>
        {node.metadata?.error && <span className="dt-node__err">{String(node.metadata.error)}</span>}
      </div>
      {children.sort((a, b) => a.startTime - b.startTime).map((c) => (
...
export function DependencyTree({ trace }: { trace: FullTrace }) {
  ...
  if (roots.length === 0) {
    ...
    return (
      <div className="dtree">
        {all.map((n) => <NodeRow key={n.id} node={n} trace={trace} depth={0} />)}
      </div>
    );
  }
  ...
  return (
    <div className="dtree">
      {roots.sort((a, b) => a.startTime - b.startTime).map((r) => (
