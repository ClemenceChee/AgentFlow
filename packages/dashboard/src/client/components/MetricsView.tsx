...
export function MetricsView({ trace }: { trace: FullTrace }) {
  ...
  const _running = nodes.filter((n) => n.status === 'running').length;
  ...
  return (
    <div className="mview">
      <div className="mv-row">
        <div className="mv-c">
          <span className="mv-v">{nodes.length}</span>
          <span className="mv-l">Nodes</span>
        </div>
        <div className="mv-c">
          <span className="mv-v c-ok">{completed}</span>
          <span className="mv-l">OK</span>
        </div>
        <div className="mv-c">
          <span className="mv-v" style={failed > 0 ? { color: 'var(--color-critical)' } : {}}>
            {failed}
          </span>
          <span className="mv-l">Fail</span>
        </div>
        <div className="mv-c">
          <span className="mv-v">{rate}%</span>
          <span className="mv-l">Success</span>
        </div>
        <div className="mv-c">
          <span className="mv-v">{fmtDur(dur)}</span>
          <span className="mv-l">Total</span>
        </div>
        <div className="mv-c">
          <span className="mv-v">{fmtDur(avg)}</span>
          <span className="mv-l">Avg</span>
        </div>
        <div className="mv-c">
          <span className="mv-v">{fmtDur(p95)}</span>
          <span className="mv-l">P95</span>
        </div>
        <div className="mv-c">
          <span className="mv-v">{fmtDur(max)}</span>
          <span className="mv-l">Max</span>
        </div>
      </div>
          ...
          <div key={t} className="mt-row">
            ...
            <span className="mt-bar">
              <span
                className="mt-fill"
                style={{
                  width: `${(s.count / nodes.length) * 100}%`,
                  background: TYPE_COLORS[t] ?? TYPE_COLORS.default,
                }}
              />
            </span>
          </div>
