...
const TYPE_COLORS: Record<string, string> = {
  ...
  extraction: '#ff7b72', sweep: '#ffa657', autofix: '#d2a8ff', default: '#8b949e',
};
...
export function FlameChart({ trace }: { trace: FullTrace }) {
  ...
  const { rows, timeStart, timeEnd, totalDur, failedNodes } = useMemo(() => {
    ...
    return { rows: r, timeStart: tS, timeEnd: tE, totalDur: total, failedNodes: nodes.filter((n) => n.status === 'failed') };
  }, [trace]);
    ...
    <div className="flame">
      ...
      {failedNodes.length > 0 && (
        <div className="flame__fail-callout">
          <div className="flame__fail-title">{'\u2718'} {failedNodes.length} Failed</div>
          {failedNodes.map((n) => (
      ...
      <div className="flame__range">
        {new Date(timeStart).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' })}
        {' \u2192 '}
        {new Date(timeEnd).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
        {' '}({fmtDur(totalDur)})
      </div>

      {/* Time axis */}
      <div className="flame__axis">
        <span>0</span><span>{fmtDur(totalDur * 0.25)}</span><span>{fmtDur(totalDur * 0.5)}</span><span>{fmtDur(totalDur * 0.75)}</span><span>{fmtDur(totalDur)}</span>
      </div>
        ...
        <div key={depth} className="flame__row">
          ...
          <div className="flame__track">
            {nodes.map((n) => {
              ...
              const width = Math.max(((( n.endTime ?? n.startTime) - n.startTime) / totalDur) * 100, 0.2);
              ...
              return (
                <div
                  ...
                  style={{ left: `${left}%`, width: `${width}%`, background: color, opacity: isHov ? 1 : 0.8 }}
                  onMouseEnter={(e) => { setHovered(n.id); setMousePos({ x: e.clientX, y: e.clientY }); }}
                ...
                >
                  ...
                  {width <= 1.5 && isHov && (
                    <span className="flame__side-label">{n.name}</span>
                  )}
                  {isHov && (
                    <div className="flame__tooltip" style={{ left: mousePos.x + 12, top: mousePos.y - 10 }}>
                      <div className="flame__tt-title">{n.name}</div>
                      <div className="flame__tt-type" style={{ color }}>{n.type}{n.metadata?.operation ? ` \u2192 ${String(n.metadata.operation)}` : ''}</div>
                      <div className="flame__tt-dur">{fmtDur((n.endTime ?? n.startTime) - n.startTime)} &middot; {n.status}</div>
