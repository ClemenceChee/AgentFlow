...
export interface GovernanceData {
  ...
  insights: Array<{
    type: string; title: string; claim: string; confidence: string;
    layer?: string; confidence_score?: number; proposal_status?: string;
  }>;
  canon: Array<{
    type: string; title: string; claim: string;
    ratified_by?: string; ratified_at?: string;
  }>;
  generatedAt?: string;
  message?: string;
}
...
export function useSomaGovernance() {
  ...
  const promote = useCallback(async (entryId: string) => {
    try {
  ...
  const reject = useCallback(async (entryId: string, reason: string) => {
    try {
