...
export function KpiCard({ metric, onClick, active }: Props) {
  ...
  return (
    <div
      className="bi-kpi"
      onClick={onClick}
      style={{ cursor: onClick ? 'pointer' : undefined, borderColor: active ? 'var(--info)' : undefined }}
    >
      ...
      <div className="bi-kpi__row">
        <span className={`bi-kpi__trend bi-kpi__trend--${isGoodTrend ? 'up' : metric.trend === 'stable' ? 'stable' : 'down'}`}>
      ...
      <div className="bi-kpi__sparkline">
        <Sparkline values={generateSparkData(metric)} color={isGoodTrend ? 'var(--ok)' : metric.trend === 'stable' ? 'var(--t3)' : 'var(--fail)'} />
      </div>
    </div>
  );
}
...
function fmtValue(value: number, unit: string): string {
  ...
  if (unit === 'ms') return value < 1000 ? Math.round(value).toString() : (value / 1000).toFixed(1) + 'k';
...
function isPositiveTrend(name: string, trend: string): boolean {
  const higherIsBetter = ['overall_success_rate', 'active_agents', 'total_executions', 'compliance_score'];
