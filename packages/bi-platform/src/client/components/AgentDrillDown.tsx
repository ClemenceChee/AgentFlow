      ...
      {data && (
        <div>
          ...
          <div className="bi-grid--3 bi-grid" style={{ marginBottom: 'var(--s3)' }}>
            <StatBox label="Success Rate" value={`${data.current.successRate.toFixed(1)}%`} color={data.current.successRate >= 90 ? 'var(--ok)' : 'var(--warn)'} />
            <StatBox label="Avg Response" value={`${data.current.avgResponseTimeMs.toFixed(0)}ms`} />
            <StatBox label="Total Executions" value={data.current.totalExecutions.toLocaleString()} />
          </div>
          ...
          {data.compliance.alerts.length > 0 && (
            <div style={{ marginBottom: 'var(--s3)' }}>
              {data.compliance.alerts.map((a, i) => (
                <div key={i} className="badge badge--fail" style={{ marginRight: 'var(--s2)', marginBottom: 'var(--s1)' }}>{a}</div>
              ))}
            </div>
          )}
          ...
          {data.history.length > 0 && (
            <div>
              <div style={{ fontSize: 'var(--xs)', color: 'var(--t3)', marginBottom: 'var(--s2)' }}>Execution History</div>
              <Sparkline values={data.history.map((h) => h.executions)} color="var(--info)" width={300} height={40} />
              <table className="bi-table" style={{ marginTop: 'var(--s2)' }}>
                <thead>
                  <tr><th>Date</th><th>Executions</th><th>Successful</th><th>Avg Duration</th><th>Error Rate</th></tr>
                </thead>
                <tbody>
                  {data.history.slice(0, 10).map((h) => (
                    <tr key={h.date}>
                      ...
                      <td style={{ color: h.errorRate > 10 ? 'var(--fail)' : 'var(--t2)' }}>{h.errorRate.toFixed(1)}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function StatBox({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div style={{ padding: 'var(--s2)' }}>
      ...
      <div style={{ fontSize: 'var(--lg)', fontWeight: 700, fontFamily: 'var(--fm)', color: color || 'var(--t1)' }}>{value}</div>
    </div>
  );
}
