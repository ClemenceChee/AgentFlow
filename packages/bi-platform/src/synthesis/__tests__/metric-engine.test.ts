...
describe('MetricEngine', () => {
  ...
  it('returns cached metric on cache hit', async () => {
    const cached = { name: 'test', value: 42, unit: 'count', trend: 'up' as const, trendPct: 5, period: '30d', calculatedAt: '' };
  ...
  it('getExecutiveKPIs returns all 6 metrics', async () => {
    ...
    db.query.mockResolvedValue({ rows: [{ total: '100', prev_total: '90', rate: '0.9', prev_rate: '0.85', avg: '150', prev_avg: '200', count: '5', prev_count: '4', compliant: '95', total_records: '100' }] });

    const kpis = await engine.getExecutiveKPIs();
