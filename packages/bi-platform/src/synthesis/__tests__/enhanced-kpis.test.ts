...
import {
  ...
  computeOperationalEffectiveness,
  computeTokenEconomics,
} from '../enhanced-kpis.js';

function createMockOpenClaw(agentData: any[] = [], tokenEcon?: any) {
  return {
    getAgentData: vi.fn(async () => agentData),
    getTokenEconomics: vi.fn(
      async () =>
        tokenEcon ?? {
          totalCost: 0.15,
          totalTokens: 50000,
          perAgent: [
            { agentId: 'main', cost: 0.1, tokens: 30000 },
            { agentId: 'test', cost: 0.05, tokens: 20000 },
          ],
          ...
          jobs: [
            {
              ...
              recentDurations: [50000, 55000, 48000],
            },
            {
              jobId: 'digest',
              totalRuns: 10,
              ...
              recentDurations: [300000, 400000, 500000],
            },
          ],
        },
    ),
...
describe('computeTokenEconomics', () => {
  it('aggregates session and cron spend', async () => {
    const econ = await computeTokenEconomics(
  ...
  it('flags wasted warning above 20%', async () => {
    ...
    const oc = createMockOpenClaw([], {
      totalCost: 1.0,
      totalTokens: 10000,
      perAgent: [{ agentId: 'main', cost: 1.0, tokens: 10000 }],
      perModel: [],
    });
...
describe('computeOperationalEffectiveness', () => {
  ...
  it('computes delegation success across sources', async () => {
    const eff = await computeOperationalEffectiveness(
      createMockOpenClaw([
        { agentId: 'main', status: 'healthy', totalMessages: 50, totalTokens: 0, totalCost: 0 },
      ]) as any,
      createMockCron() as any,
